...
# ****** I believe genderidentity doesnt exist in the data set and that it is gender *******

tbl_clean <- select(tbl, session_id, gender, raceomb_002, D_biep.White_Good_all,Mn_RT_all_3467,politicalid_7,
                    edu_14, STATE,att_7,tblacks_0to10,twhites_0to10,labels)
...
#I kept gender as is, because it is already named gender in the dataset.

tbl_clean <- rename(tbl_clean, id = session_id, race = raceomb_002, bias = D_biep.White_Good_all,rt = Mn_RT_all_3467,
                    edu = edu_14, pol = politicalid_7, state = STATE, att = att_7, temp_b = tblacks_0to10, temp_w = twhites_0to10)
...
#charTOfactor <- c('id', 'gender', 'state')

## The one below is longer than what we can do with a single line of code:
#tbl_clean <- as_factor(tbl_clean, c('id', 'gender', 'state'))
# tbl_clean$id <- as.factor(tbl_clean$id)
# tbl_clean$gender <- as.factor(tbl_clean$gender)
# tbl_clean$state <- as.factor(tbl_clean$state)
...
# white men
whiteMen <- filter(tbl_clean, gender4 == 'M' & race == 6)
print(mean(whiteMen$bias))

# white women
whiteWomen <- filter(tbl_clean, gender4 == 'F' & race == 6)
print(mean(whiteWomen$bias))

# advanced degree holders who are men
adMen <- filter(tbl_clean, gender4 == 'M' & edu7 == 'advanced degree')
print(mean(adMen$bias))

# high school graduates who are men
hsMen <- filter(tbl_clean, gender4 == 'M' & edu7 == 'highschool graduate')
print(mean(hsMen$bias))
